...
for method in integration_methods:
    ...
    # fig, axes = plt.subplots(2, 5, figsize=(15, 8), dpi=300, gridspec_kw={'wspace': 0.5})  # 3 rows, 4 columns
    fig, axes = plt.subplots(
    2, 5,
    figsize=(15, 8),   # slightly reduced height for ratio
    dpi=300,
    ...
    ax.set_title('Dry weight', fontsize = title_font)  # Title for each subplot
    ax.set_ylabel('$y_{\mathrm{dw}} \ \ [\mathrm{g}\!\cdot\!\mathrm{m}^{-2}]$', fontsize = label_font)  # Display ylabel
    ...
    ax.set_title('CO$_{2}$ Concentration', fontsize = title_font)  # Title for each subplot
    ax.set_ylabel('$y_{\mathrm{CO}_{2}} \ \ [\mathrm{ppm}]$', fontsize = label_font)  
    ...
    ax.set_title('Air temperature', fontsize = title_font)  # Title for each subplot
    ax.set_ylabel('$y_{\mathrm{T}} \ \ [^o\mathrm{C}]$', fontsize = label_font)  # Display ylabel
    # ax.legend(loc='best', fontsize = 12.0) # Display legend
    ax.grid(True)  # Add grid
    # ax.set_xlim(start, end)
    # ax.set_xlabel('Time [Days]', fontsize = 14.0)
    ...
    ax.set_title('Relative Humidity', fontsize = title_font)  # Title for each subplot
    ax.set_ylabel('$y_{\mathrm{RH}} \ \ [\%]$', fontsize = label_font)  # Display ylabel
    # ax.legend(loc='best', fontsize = 12.0) # Display legend
    ax.grid(True)  # Add grid
    # ax.set_xlim(start, end)
    # ax.set_xlabel('Time [Days]', fontsize = 14.0)
    ...
    axes[0, 4].legend(handles=handles, loc='center', fontsize=legend_font, frameon=False)  # Add legend to the center
    
    # Plot weather conditions
    ax = axes[1, 0]
    ax.plot(x, d_ol[method]['Irradiance[W· m^{−2}]'], color='black', linewidth=d_lineWidth)  # Example plot
    ax.set_title('Incoming radiation', fontsize = title_font)  # Title for each subplot
    ax.set_ylabel('$d_{\mathrm{I}} \ \ [\mathrm{W}\!\cdot\!\mathrm{m}^{-2}]$', fontsize = label_font)  # Display legend
    ax.grid(True)  # Add grid
    ...
    ax.set_title('CO$_{2}$ supply rate ', fontsize = title_font)  # Title for each subplot
    ax.set_ylabel('$u_{\mathrm{CO}_{2}} \ \ [\mathrm{mg}\!\cdot\!\mathrm{m}^{-2}\!\cdot\!\mathrm{s}^{-1}]$', fontsize = label_font)  # Display ylabel
    ax.grid(True)  # Add grid
    ax.set_xlabel('Time [Days]', fontsize = label_font)
    
    ax = axes[1, 2]
    ax.plot(x[:Nsim], u_opt[method]['Ventilation'], color='darkgreen', linewidth=lineWidth, linestyle='-') 
    ax.set_title('Ventilation rate', fontsize = title_font)  # Title for each subplot
    ax.set_ylabel('$u_{\mathrm{vent}} \ \ [\mathrm{L}\cdot\mathrm{m}^{-2}\cdot\mathrm{s}^{-1}]$', fontsize = label_font)  # Display ylabel
    ax.grid(True)  # Add grid
    # ax.set_xlim(start, end)
    ax.set_xlabel('Time [Days]', fontsize = label_font)
    ...
    ax.set_title('Heating energy supply', fontsize = title_font)  # Title for each subplot
    ax.set_ylabel('$u_{\mathrm{heat}} \ \ [\mathrm{W}\!\cdot\!\mathrm{m}^{-2}]$', fontsize = label_font)  # Display ylabel
    ax.grid(True)  # Add grid
    # ax.set_xlim(start, end)
    ax.set_xlabel('Time [Days]', fontsize = label_font)
    ...
    ax.set_title('Air Exchange Flow', fontsize = title_font)  # Title for each subplot
    ax.set_ylabel('$u_{\mathrm{flow}} \ \ [\mathrm{L}\cdot\mathrm{m}^{-2}\cdot\mathrm{s}^{-1}]$ ', fontsize = label_font)  # Display ylabel
    ax.grid(True)  # Add grid
    # ax.set_xlim(start, end)
    ax.set_xlabel('Time [Days]', fontsize = label_font)
    ...
    # plt.tight_layout()
    fig.subplots_adjust(
    left=0.03,      # reduce left whitespace
    right=0.99,     # reduce right whitespace
    top=0.96,       # reduce top whitespace
    bottom=0.10     # leave room for x-labels
)
    ...
    if storage:
        ...
        filename,
        bbox_inches="tight",
        pad_inches=0.02     # extremely small padding
    )
