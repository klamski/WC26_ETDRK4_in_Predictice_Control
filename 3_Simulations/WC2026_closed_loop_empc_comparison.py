...
for j in range(Nsim):
    ...
    u_cl[:,j] = u_opt[:,0]
    solver_status.append(solver.stats()["return_status"])
    
    # Update initial condition
    x0 = x_cl[:, j+1]
    
    # Update initial guess
    u_guess = np.hstack((u_opt[:,1:], u_opt[:, -1].reshape(-1, 1)))
...
axs[0, 0].plot(time, y_cl[0, :], color="C0", label="Optimal")
axs[0, 0].set_title("Dry Weight [g/m^2]")
beautify(axs[0, 0])

axs[0, 1].axhline(1600, color="k", linestyle="--", alpha=0.5, linewidth=0.8)
axs[0, 1].plot(time, y_cl[1, :] * 1e3, color="C0", label="Below")
axs[0, 1].plot(time, y_cl[4, :] * 1e3, color="C1", label="Above")
...
axs[0, 2].plot(time, d_ol[2, : Nsim + 1], color="C4", label="Out")
...
rh_out = (
    params[11]
    * (d_ol[2, : Nsim + 1] + params[12])
    / (11 * np.exp(params[26] * d_ol[2, : Nsim + 1] / (d_ol[2, : Nsim + 1] + params[27])))
    * d_ol[3, : Nsim + 1]
    * 1e2
)
axs[0, 3].axhline(80, color="k", linestyle="--", alpha=0.5, linewidth=0.8)
axs[0, 3].plot(time, y_cl[3, :], color="C0", label="Below")
axs[0, 3].plot(time, y_cl[6, :], color="C1", label="Above")
...
# === BOTTOM ROW ===
axs[1, 0].plot(time, d_ol[0, : Nsim + 1] / 1e3, color="C4")
axs[1, 0].set_title("Irradiance [W/m^2]")
axs[1, 0].set_xlabel("Time (t)")
beautify(axs[1, 0])

axs[1, 1].plot(time[:-1], u_cl[0, :], drawstyle="steps-post", color="C2")
axs[1, 1].set_title("CO$_2$ Supply [mg/m^2/s]")
axs[1, 1].set_xlabel("Time (t)")
beautify(axs[1, 1])

axs[1, 2].plot(time[:-1], u_cl[2, :], drawstyle="steps-post", color="C2")
axs[1, 2].set_title("Heating [W/m^2]")
axs[1, 2].set_xlabel("Time (t)")
beautify(axs[1, 2])

axs[1, 3].plot(time[:-1], u_cl[1, :], drawstyle="steps-post", color="C2")
axs[1, 3].set_title("Ventilation [L/m^2/s]")
axs[1, 3].set_xlabel("Time (t)")
beautify(axs[1, 3])

axs[1, 4].plot(time[:-1], u_cl[3, :], drawstyle="steps-post", color="C2")
...
d_ol_df = pd.DataFrame(np.hstack((d_ol[:,0:Nsim+1].T, rh_out.reshape(-1, 1))), columns=d_labels)
simDetails_pd = pd.DataFrame([simDetails], columns=simDetail_labels)


if storage:
    u_cl_df.to_csv(f'../4_OutputData/closedLoop-ENMPC/{integration_method}_ExtvanHentenModel_ENMPC_clInputs.csv', index=False)
